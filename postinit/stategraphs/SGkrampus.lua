...
env.AddStategraphPostInit("krampus", function(inst)
    ...
    if attackedeventhandler then
        ...
        attackedeventhandler.fn = function(inst, data, ...)
            if not (inst.components.health and inst.components.health:IsDead() or inst.sg:HasStateTag("nointerrupt")) then
                if CommonHandlers.TryElectrocuteOnAttacked(inst, data) then
                    return
                elseif not (inst.sg:HasAnyStateTag("attack", "electrocute") or CommonHandlers.HitRecoveryDelay(inst)) then -- Krampus can interrupt being electrocuted at the time of typing this.
    ...
    if doattackeventhandler then
        ...
        doattackeventhandler.fn = function(inst, ...)
