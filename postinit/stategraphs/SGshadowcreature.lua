...
env.AddStategraphPostInit("shadowcreature", function(inst)
    ...
    local _DoAttack = inst.events["doattack"].fn -- t/n beaks have a special attack state
    inst.events["doattack"].fn = function(inst, data, ...)
        if not (inst.sg:HasStateTag("busy") or inst.components.health and inst.components.health:IsDead()) then
            if inst.prefab == "nightmarebeak" or inst.prefab == "terrorbeak" then
                inst.sg:GoToState("terrorattack", data.target)            
            else
                _DoAttack(inst, data, ...)
            end
        end
    ...
    -- crawling h/n have a special hit state
    local hitstate = inst.states["hit"]
    if hitstate then
        local hitstate_onenter = inst.states["hit"].onenter
        hitstate.onenter = function(inst, ...)
            if inst:HasTag("crawlinghorror") and not inst._cdtask then
                inst._cdtask = inst:DoTaskInTime(1, function(inst) inst._cdtask = nil end)
                for i = 0, 8, 2 do inst:DoTaskInTime((i > 0 and i or 1) * FRAMES, function(inst) inst:LaunchProjectile() end) end
            end
            return hitstate_onenter(inst, ...)
        end
    end
