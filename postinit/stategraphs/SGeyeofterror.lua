...
env.AddStategraphPostInit("eyeofterror", function(inst)
    ...
    inst.events["charge"].fn = function(inst)
        if not inst.components.health:IsDead() and not inst.components.freezable:IsFrozen() and not inst.components.sleeper:IsAsleep() and not inst.sg:HasStateTag("busy") then
            if inst.prefab == "eyeofterror" then
                inst.sg:GoToState(math.random() > .5 and "charge_pre" or "shoot_pre", inst.components.combat.target)
            else
                local x, y, z = inst.Transform:GetWorldPosition()
                local twin = TheSim:FindEntities(x, y, z, 50, {"eyeofterror"})
                if twin and #twin < 2 then
                    inst.sg:GoToState(math.random() > .5 and "charge_pre" or "shoot_pre", inst.components.combat.target)
                else
                    if inst.canshoot then
                        for i, v in pairs(twin) do
                            if v ~= inst then
                                v.canshoot = false
                                break
                            end
    ...
    local states =
    {
        State{
            ...
            onenter = function(inst, target)
                ...
                local cooldown = inst.sg.mem.transformed and inst._cooldowns.mouthcharge or inst._cooldowns.charge
            ...
            events =
            {
                EventHandler("animover", function(inst)
                    if inst.sg.mem.transformed then
                        inst.sg.mem.mouthcharge_count = inst.prefab == "eyeofterror" and 2 or math.random(3, 5)
        ...
        State{
            ...
            {
                EventHandler("animover", function(inst)
                    inst.sg.mem.mouthcharge_count = not inst.sg.mem.mouthcharge_count and 0 or inst.sg.mem.mouthcharge_count - 1
                    inst.sg:GoToState(inst.sg.mem.mouthcharge_count and inst.sg.mem.mouthcharge_count > 0 and "mouthshoot_loop" or "shoot_pst")
                end),
            },
        },
