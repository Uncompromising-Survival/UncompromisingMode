...
local function BounceStuff(inst) --Possibly upvaluehack this function in the future?
    if not inst.sg.mem.recentlybounced then
    ...
    for i, v in ipairs(ents) do
        if v:IsValid() and not (v.components.inventoryitem.nobounce or inst.sg.mem.recentlybounced[v]) and v.Physics and v.Physics:IsActive() then
            ...
            onenter = function(inst)
                ...
                inst:DoTaskInTime(0.5,function()
                    ...
                    if target then
                        ...
                        if inst.sg.statemem.startpos then
