...
local function FaceTarget(inst)
    ...
    if inst.sg.mem.focustargets then
        local mindistsq = math.huge
        for i = #inst.sg.mem.focustargets, 1, -1 do
            local v = inst.sg.mem.focustargets[i]
            if v:IsValid() and not (v.components.health and v.components.health:IsDead()) and
...
local function SpinWall(inst, speed)
    if inst.defensebees then
        for i, bee in ipairs(inst.defensebees) do
            if not (bee.components.health and bee.components.health:IsDead()) and bee.components.linearcircler then
...
local function Honey(inst, angle, count, variation, slowness)
    ...
    if TheWorld.Map:IsPassableAtPoint(x, 0, z) then
        ...
        if inst.prefab == "cherry_beequeen" or inst.prefab == "cherry_honey_trail" then
            fx.slowness = inst.flowerbuffs and fx.slowness + inst.flowerbuffs.honeyslowness or slowness
        end
...
local function SpillHoney(inst)
    ...
    local variations = {-2, 2, 0}
    for i = 1, 3 do
        Honey(inst, angle, 10, variations[i])
    end
end
        ...
        State{
            ...
            timeline =
            {
                TimeEvent(16 * FRAMES, function(inst)
                    inst.SpawnDefensiveBees(inst, inst.components.health:GetPercent() < 0.5)
                end),
            ...
            timeline =
            {
                TimeEvent(16 * FRAMES, function(inst)
                    ...
                    if inst.bonuswall then
                        inst.bonuswall = nil
                        inst.SpawnDefensiveBees(inst, true)
                    end
                end),
                CommonHandlers.OnNoSleepTimeEvent(32 * FRAMES, function(inst)
                    inst.sg:RemoveStateTag("busy")
        ...
        State{
            ...
            onexit = function(inst)
                ...
                if inst.seeker_hitlist and #inst.seeker_hitlist > 1 then
                    ...
                    if possibletarget and not (possibletarget.components.health and possibletarget.components.health:IsDead())
                        and not possibletarget:HasTag("playerghost") then
                ...
                inst:DoTaskInTime(0, function(inst)
                    if inst.seekercount > 0 then
                        ...
                        inst.tiredcount = 12
                        if not (inst.components.health and inst.components.health:IsDead()) then inst.sg:GoToState("tired_pre") end
                    end
                end)
            end,
        },
