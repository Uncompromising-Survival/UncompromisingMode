...
env.AddStategraphPostInit("SGbeeguard", function(inst) -- beeguard time
    ...
    local states = {
        ...
        State{
            ...
            onexit = function(inst) StartCollide(inst) end
        },
        State{
        ...
        State{
            name = "stab",
            tags = {"busy", "nosleep", "nofreeze", "attack", "noattack", "noelectrocute", "mortar"}, -- We don't want the beeguard to try and attack, but we do need to let the game know this is an attacking state.
        ...
        State{
            ...
            timeline =
            {
                TimeEvent(10 * FRAMES, function(inst) StopBuzz(inst) end)
            },
        ...
        State{
            ...
            onupdate = function(inst) if inst.components.linearcircler and inst.components.linearcircler.distance < 12 then inst.components.linearcircler.distance = 0.1 + inst.components.linearcircler.distance end end
        },
        State{
        ...
        State{
            ...
            onupdate = function(inst) if inst.holdPoint then inst.Transform:SetPosition(inst.holdPoint.x, inst.holdPoint.y, inst.holdPoint.z) end end,
        ...
        State{
            ...
            onexit = function(inst)
                ...
                if inst.rallyPoint and inst:IsValid() and inst:GetDistanceSqToPoint(inst.rallyPoint) > 1 then
                    -- inst:DoTaskInTime(.05, function(inst) TheNet:Announce("Tried to go to"..inst.sg.currentstate.name) end)
                    inst:DoTaskInTime(.1, function(inst) inst.sg:GoToState("rally_at_point") end)
                end
            end
        },
        State{
            ...
            timeline =
            {
                ...
                TimeEvent(21 * FRAMES, function(inst) inst.sg:RemoveStateTag("busy") end)
            },
            ...
            events =
            {
                EventHandler("animover", function(inst) inst.sg:GoToState("hold_position_ring") end)
            }
        }
    }
