...
local function Wallcheck(inst)
    if inst.defensebees then
        for i, bee in ipairs(inst.defensebees) do
            if not (bee.components.health and bee.components.health:IsDead()) then
                return true
            end
        end
    end
    ...
    if not (inst.components.health and inst.components.health:IsDead()) then
        ...
        if inst.components.health and inst.components.health:GetPercent() < 0.75 then
            ...
            if data.attacker and data.attacker.components.combat and inst.stompready then
                ...
                if TheWorld.Map:GetPlatformAtPoint(x, z) then
                    inst.stomprage = inst.stomprage + 20
                end
                if inst.stomprage > 40 then
                    inst:PushEvent("stomp", {attacker = data.attacker})
                end
            end
        end
    end
...
local function StompRageCalmDown(inst)
    inst.stomprage = inst.stomprage < 3 and 0 or inst.stomprage - 3
end
...
local function KillOffOldDefense(inst)
    for i, bee in ipairs(inst.defensebees) do
        if not (bee.components.health and bee.components.health:IsDead()) and bee.components.timer:TimerExists("natural_death") then
            bee.components.timer:SetTimeLeft("natural_death", math.random(0, 2))
        end
    end
end

local function SpawnDefensiveBees(inst, two)
    local x, y, z = inst.Transform:GetWorldPosition()
    local LIMIT = two and 5 or 4
    if inst.defensebees then
    ...
    for i = 1, 8 do
        ...
        inst.defensebees[i].components.linearcircler.randAng = i * 0.125 * (two and 3 or 4) / 5
        ...
        inst.defensebees[i].components.linearcircler.setspeed = two and 0.05 or 0
