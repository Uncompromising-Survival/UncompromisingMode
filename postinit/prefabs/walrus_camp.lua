...
local function CupcakeTheHound(guard, camp)
	...
	guard:DoPeriodicTask(0.5, function()
		if not (guard and guard:IsValid()) then
			return
		end
...
local function AllHoundsGoToHeaven(inst)
	...
	inst._flare_return_task = inst:DoPeriodicTask(0.5, function()
...
local function HowIsTheHuntGoing(inst)
	...
	inst._flare_state_task = inst:DoPeriodicTask(0.5, function()
		if not inst:IsValid() then
			return
		end
		...
		if inst._flare_glacial ~= nil and not inst._flare_glacial:IsValid() then
			Replacement(inst)
			return
		end
	end)
