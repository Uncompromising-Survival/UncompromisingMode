...
local HOUND_LIGHNTING_CANT_TAGS = {"moose", "mossling"}
        ...
        inst.stormtask = inst:DoPeriodicTask(1, function(inst)
            if not inst.components.health:IsDead() then
                ...
                if inst.stormtaskcounter > 20 then
                    ...
                    if target and math.random() > 0.95 then
                    ...
                    UMCommonFns.SpawnHoundLightning(inst, {pos = pos, canttags = HOUND_LIGHNTING_CANT_TAGS})
                end
            end
        end)
    end
