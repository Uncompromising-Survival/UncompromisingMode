...
local ROCKS_LIST = {"rock1", "rock2", "rock_flintless", "springrock1", "springrock2"}
        ...
        if math.random() < chance then
            if days_survived <= 30 then
                ...
                for k = 1, (days_survived <= 30 and 1) or math.random(days_survived <= 80 and 3 or 6) do
                    ...
                    if target and table.contains(ROCKS_LIST, target.prefab) then
...
local function NewCallBack(inst, worker, workleft, ...)
    ...
    if x then
        if inst.crab and inst.crab:IsValid() then -- If crab then use his position instead
    ...
    return _OnWork and _OnWork(inst, worker, workleft, ...)
end
...
for i,v in ipairs(rocks) do
    env.AddPrefabPostInit(v, function(inst)
        ...
        _OnWork = inst.components.workable.onwork
