...
env.AddComponentPostInit("hounded", function(self)
    ...
    local function GetSpawnPrefab(upgrade)
        ...
        if do_seasonal_spawn then
            if _spawnwintervariant then
                --old spawn chance was was 50%, it was a randomly picked between 2 in a table.
                --TEMP UNTIL WE PUSH VARGLETS TO LIVE.
                if TheWorld.state.iswinter and _spawndata.winter_prefab2 ~= nil and math.random() > 0.5 then
                    return _spawndata.winter_prefab2
                end
            ...
            if _spawnsummervariant then
                --old spawn chance was was 50%, it was a randomly picked between 2 in a table.
                --TEMP UNTIL WE PUSH VARGLETS TO LIVE.
                if TheWorld.state.issummer and _spawndata.winter_prefab2 ~= nil and math.random() > 0.5 then
                    return _spawndata.summer_prefab2
                end

                if (TheWorld.state.issummer or TheWorld.state.isautumn) then
    ...
    local function SummonSpawn(pt, upgrade, radius_override)
        ...
        local spawn_pt = (prefab == "magmahound" or prefab == "glacialhound" or prefab == "sporehound") and GetMagmaSpawnPoint(pt) or GetSpawnPoint(pt, radius_override)
...
env.AddPrefabPostInit("forest", function(inst)
    ...
    if inst.components.hounded ~= nil then
        ...
        if TUNING.DSTU.GLACIALHOUNDS then
            spawndata.winter_prefab2 = "glacialhound" --TEMP UNTIL WE PUSH VARGLETS TO LIVE.
        end
        if TUNING.DSTU.MAGMAHOUNDS then
            spawndata.summer_prefab2 = "magmahound" --TEMP UNTIL WE PUSH VARGLETS TO LIVE.
        end
