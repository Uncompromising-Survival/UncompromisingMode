...
function d_create_um_scrapbookdata(print_missing_icons)
    if not TheWorld.state.isautumn or TheWorld.state.israining then
        ...
        scheduler:ExecuteInTime(0.05, ExecuteConsoleCommand, nil, string.format("d_create_um_scrapbookdata(%s)", tostring(print_missing_icons or "")))
        return
    end
    ...
    for entry, _ in pairs(scrapbookprefabs) do
        ...
        if t.Transform == nil then
            print(string.format("[!!!!]  Aborting data creation command! Entry [%s] is not a valid prefab! (Lacks Transform!)", entry))
            return
        end
        if t.AnimState == nil then
            print(string.format("[!!!!]  Aborting data creation command! Entry [%s] is not a valid prefab! (Lacks AnimState!)", entry))
            return
        end
