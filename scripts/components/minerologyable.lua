...
local function set_durability_client(self, durability)
    local inst = self.inst
    print("set_durability_client", durability)
    inst.replica.minerologyable:SetDurabilityClient(durability)

    if durability <= 0 then
        self.enchant = nil
        self.enchantnum = 0

        --remove any event callbacks that might've gotten set.
        if inst.OnInventoryStateChangedGemology then
            inst:RemoveEventCallback("itemget", inst.OnInventoryStateChangedGemology)
            inst:RemoveEventCallback("itemlose", inst.OnInventoryStateChangedGemology)
            inst:RemoveEventCallback("stacksizechange", inst.OnInventoryStateChangedGemology)
        end
    end
end
...
local Minerologyable = Class(function(self, inst)
        ...
        self.inst:ListenForEvent("perishchange", function(inst, data)
            local durability = inst.components.minerologyable.durability
            local new_durability = data.percent

            if durability > new_durability then
                inst.components.minerologyable.durability = new_durability
            end
        end)
    end,
    nil,
    {
        ...
        durability = set_durability_client
    })
...
function Minerologyable:DoDelta(num, dobreakable)
    print("num", num)
    print("dobreakable", dobreakable)
    if self:IsItemUnbreakable() or dobreakable then
        self.durability = math.max(self.durability + num, 0)
    end
end

function Minerologyable:HasDurabilityEnabled()
    return self.enchant ~= nil
end

function Minerologyable:IsItemUnbreakable()
    local item = self.inst
    return not item.components.perishable and not item.components.fueled and not item.components.finiteuses and not item.components.armor
end
...
local function SendTheWilson(inst, attacker, target)
    ...
    inst.components.minerologyable:DoDelta(-0.05)
end
...
local function MakeYellow1(self, tier)
    ...
    inst:DoPeriodicTask(1, function(inst)
        inst.components.minerologyable:DoDelta(-0.0125)
    end)
...
-- um_static_mod is referenced in a postinit to the combat component in init_gemology, setting the value here will talk to the function there,
-- and make it increase damage if the player has a modifier already
local function MakeYellow2(self, tier)
    self.enchantnum = 4
    self.static = true
...
-- um_peerless_mod is referenced in a postinit to the combat component in init_gemology, setting the value here will talk to the function there,
-- and make it increase damage if the player has a modifier already
local function MakeClear1(self, tier)
    self.enchantnum = 5
...
--TODO: Check if recipe has recipe and has the nounlock data!!
local cant_be_proto = {
    "ruins_bat",
    "tentaclespike",
    "glasscutter",
...
local function Devour(inst, owner, target)
    if inst.tier ~= 1 and target:HasOneOfTags(devour_tags) and math.random() > 0.75 then -- arbitrarily said "a chance", I have no idea how common this should be
        ...
        inst.components.minerologyable:DoDelta(-0.025)
    end
...
local function OnDropedIfDeadGiveBack(inst) -- This is the only one that has an "ondropped" effect
    ...
    if owner and owner.components.health:IsDead() then -- If this happens, the owner has just died.
        ...
        inst.components.minerologyable:DoDelta(-1 / inst.tier + 1)
    end
...
local function UpdateSanityStat(inst, count)
    ...
    if inst.gem_durability_task ~= nil then
        inst.gem_durability_task:Cancel()
        inst.gem_durability_task = nil
    end

    inst.gem_durability_task = inst:DoPeriodicTask(5, function(inst)
        inst.components.minerologyable:DoDelta((-0.0125 / 2) * count)
    end)
end
