...
return Class(function(self, inst)
    ...
    local function StopTracking(player, params, ent)
        ...
        if params.targetpop ~= #params.ents then
            if params.spawntask == nil then
                params.spawntask = player:DoTaskInTime(TUNING.SANITYMONSTERS_SPAWN_INTERVAL +
                    TUNING.SANITYMONSTERS_SPAWN_VARIANCE * math.random(), UpdateSpawn, params)
            end
    ...
    UpdateSpawn = function(player, params)
        if params.targetpop > #params.ents then
            ...
            local bosses = TheSim:FindEntities(x, y, z, 35, { "_combat" }, { "FX", "NOCLICK", "INLIMBO", "player" },
                { "epic", "shadowchesspiece" })
            local bossesnearby = #
                TheSim:FindEntities(x, y, z, 30, { "_combat" }, { "FX", "NOCLICK", "INLIMBO", "player" },
                    { "deerclops", "shadowchesspiece", "mock_dragonfly", "minotaur", "stalker", "bearger", "creepingfear",
                        "mothergoose" })
            ...
            local function IsTargetingPlayer(boss)
                ...
                if target ~= nil then
                    return target ~= nil and target:HasTag("player") or target:HasTag("companion") or
                        target:HasTag("abigail")
                else
                    return false
                end
            end
    ...
    local function UpdatePopulation(player, params)
        ...
        if is_insanity_mode and player.components.sanity.inducedinsanity then
            ...
            params.poptask = player:DoTaskInTime(is_insanity_mode and
                (TUNING.SANITYMONSTERS_POP_CHANGE_INTERVAL + TUNING.SANITYMONSTERS_POP_CHANGE_VARIANCE * math.random())
    ...
    local function OnPlayerJoined(inst, player)
        ...
        if TUNING.DSTU.UPDATE_CHECK then
            if TheNet:GetServerPlaystyle() ~= nil and TheNet:GetServerPlaystyle() == "relaxed" or
                TheNet:GetServerPlaystyle() == "RELAXED" then
