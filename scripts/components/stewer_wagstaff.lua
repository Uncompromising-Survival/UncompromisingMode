...
local Stewer_Wagstaff = Class(function(self, inst)
    self.inst = inst
...
function Stewer_Wagstaff:OnRemoveFromEntity()
...
function Stewer_Wagstaff:StartCooking(doer)
    if self.targettime == nil and self.inst.components.container ~= nil then
		...
		local cpot_ingredients = self.ingredient_prefabs
		local todays_ingredients = self.inst.todays_ingredients
		-- self.product reassignment 
		
		
		-- Did you make today's recipe
		for i,v in ipairs(todays_ingredients) do
			local exitcond = false
			for j,k in ipairs(cpot_ingredients) do
				if v == k or v.."_cooked" == k or "cooked"..v == k and not exitcond then
					table.remove(cpot_ingredients,j)
					exitcond = false
				end
			end
		end
		
		if #cpot_ingredients == 0 then
			self.product = self.inst.todays_dish
		else
...
function Stewer_Wagstaff:StopCooking(reason)
    if self.task ~= nil then
        self.task:Cancel()
        self.task = nil
    end
...
function Stewer_Wagstaff:OnSave()
...
function Stewer_Wagstaff:OnLoad(data)
...
function Stewer_Wagstaff:GetDebugString()
...
function Stewer_Wagstaff:Harvest(harvester)
    if self.done then
...
function Stewer_Wagstaff:LongUpdate(dt)
