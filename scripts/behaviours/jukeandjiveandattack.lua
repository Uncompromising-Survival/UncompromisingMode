JukeAndJiveAndAttack = Class(BehaviourNode, function(self, inst, max_chase_time, give_up_dist, max_attacks, findnewtargetfn, walk, distance_from_ocean_target)
    BehaviourNode._ctor(self, "JukeAndJiveAndAttack")
    self.inst = inst
...
function JukeAndJiveAndAttack:Visit()
    ...
    if self.status == RUNNING then
        ...
        elseif combat.target.components.health ~= nil and combat.target.components.health:IsDead() then
			...
			local angle = (self.inst:GetAngleToPoint(target_position) + 180) - 105
