...
local function findboattoclamp(inst)
	if inst.boat then
		return Vector3(inst.boat.Transform:GetWorldPosition())
	else
		local x,y,z = inst.Transform:GetWorldPosition()
...
local Um_Ocupus_TentacleBrain = Class(Brain, function(self, inst)
    Brain._ctor(self, inst)
end)

function Um_Ocupus_TentacleBrain:OnStart()
    local root = PriorityNode(
    {
       WhileNode(function() return not (self.inst.sg:HasStateTag("clampped") or self.inst.sg:HasStateTag("coming"))  end, "not clamping",
        PriorityNode({
