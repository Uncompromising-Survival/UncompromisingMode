...
local function updateclaustrophobia(inst)
	...
	if not TheWorld.ismastersim or not TheNet:IsDedicated() then
		if inst._claustrophobiacdtask == nil then
			...
			local ents = TheSim:FindEntities(x, y, z, 6, { "_health", "_combat" }, { "noclaustrophobia", "structure", "wall", "fx", "NOCLICK", "INLIMBO", "invisible", "player", "playerghost", "ghost", "shadow", "shadowcreature", "shadowminion", "stalkerminion", "shadowchesspiece", "boatbumper", "spore", "pigelite", "oceanfishable", "trap", "companion" } )
