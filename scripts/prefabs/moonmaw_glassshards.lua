...
local function destroystuff(inst)
    if not inst.WINDSTAFF_CASTER then
        inst:Remove()
    end
        ...
        for i, v in ipairs(ents) do
            if v ~= inst.WINDSTAFF_CASTER and v:IsValid() then
                if v.components.health and not v.components.health:IsDead()
                    and v.components.combat and v.components.combat:CanBeAttacked() then
                    local damage = 40
...
local function RangeAttack(inst)
    local doer = inst.WINDSTAFF_CASTER
    local target = doer and (inst.MOONMAW_TARGET and inst.MOONMAW_TARGET:IsValid() and inst.MOONMAW_TARGET or doer.components.combat and doer.components.combat.target)
    if target and target:IsValid() then
        local x, y, z = inst.Transform:GetWorldPosition()
        local shardattack = SpawnPrefab("moonmaw_glassshards")
        shardattack.Transform:SetPosition(x, y, z)
        shardattack.anim = inst.anim
        shardattack.components.projectile:Throw(doer, target, doer)
    end
    inst:Remove()
end
...
local function shard_fn()
    ...
    light:SetColour(121 / 255 ,235 / 255, 12 / 255)
    ...
    inst.components.projectile:SetLaunchOffset(Vector3(0, .5, 0))
