...
local function vestigefn()
    ...
    inst.components.inventoryitem.grabbableoverridetag = "vetcurse"
...
local function toground(inst)
    inst.persists = false
    if inst._task == nil then
        inst._task = inst:DoTaskInTime(120 * (0.5 + math.random()), KillVetSoul) -- NOTES(JBK): This is 1.1 max keep it in sync with "[WST]"
    end
