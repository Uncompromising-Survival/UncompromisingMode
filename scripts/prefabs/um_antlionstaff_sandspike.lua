-- this is only to stop patching aroun vanillas prefab both spike ambush and bastion spawn from here
local assets =
{
...
-- castles only, spikes never register it
...
local function MakeSpikeFn(shape, size)
    ...
    return function()
        ...
        if isblock then
            ...
            if size == nil then
                inst:SetPrefabName("um_antlionstaff_sandspike_"..inst.spikesize)
            end
        end
        --points name and inspect description at vanillas
        inst:SetPrefabNameOverride(isblock and "sandblock" or "sandspike")
        ...
        -- castles keep vanillas physics but spikes get none at all
        if isblock then
        ...
        inst.scrapbook_proxy = isblock and "sandblock" or "sandspike"
        ...
        inst.scrapbook_anim = isblock and "block_pst" or "tall_pst"
...
for i, v in ipairs(SPIKE_SIZES) do
    local name = "um_antlionstaff_sandspike_"..v
    table.insert(prefabs, name)
    table.insert(ret, Prefab(name, MakeSpikeFn("spike", v), assets, { "glass_fx", "glassspike_"..v }))
end
table.insert(ret, Prefab("um_antlionstaff_sandspike", MakeSpikeFn("spike"), assets, prefabs))

table.insert(ret, Prefab("um_antlionstaff_sandblock", MakeSpikeFn("block"), block_assets, { "glass_fx", "glassblock" }))
