...
local function OnUpdate(inst)
    ...
    local um_miasmamanager = TheWorld.components.um_miasmamanager
    local nearestfire = TheSim:FindEntities(x, y, z, FIRE_RADIUS, FIRE_MUST_TAGS)[1]
	local light = TheSim:GetLightAtPoint(x, y, z, .1)
	
	if light > .3 then
        if inst._miasma_kill_task == nil then
			print("task == nil")
            inst._miasma_kill_task = 1
		else
			print("Light kill value on miasma == "..(.5 * (1 + light)))
            inst._miasma_kill_task = inst._miasma_kill_task + (.5 * (1 + light))
			
			if not inst.SoundEmitter:PlayingSound("smolder") then
				inst.SoundEmitter:PlaySound("dontstarve_DLC001/summer/smolder", "smolder")
			elseif inst.SoundEmitter:PlayingSound("smolder") and inst._miasma_kill_task > 10 then
				inst.SoundEmitter:KillSound("smolder")
				inst.SoundEmitter:PlaySound("dontstarve/common/fireOut")
			end
			
			if inst._miasma_kill_task > 10 then
				inst:Remove()
			end
        end
        inst._diminishing:set(true)
    else
		if inst.SoundEmitter:PlayingSound("smolder") then
			inst.SoundEmitter:KillSound("smolder")
		end
	...
	--[[
    if light > .3 then
        if um_miasmamanager and um_miasmamanager:GetMiasmaAtPoint(x, y, z) then
			print("set diminish at point")
            um_miasmamanager:SetMiasmaDiminishingAtPoint(x, y, z, false)
        elseif inst._miasma_kill_task == nil then
			...
			if not inst.SoundEmitter:PlayingSound("smolder") then
				if inst.um_shadowcloaked_light == nil then
					local light_emitter = SpawnPrefab("um_shadowcloaked_light")
					light_emitter.entity:SetParent(inst.entity)
					light_emitter._owner = inst
						
					inst.um_shadowcloaked_light = light_emitter
					inst:ListenForEvent("onremove", onremovelight, inst.um_shadowcloaked_light)
				end
					
				inst.SoundEmitter:PlaySound("dontstarve_DLC001/summer/smolder", "smolder")
			elseif inst.SoundEmitter:PlayingSound("smolder") and inst._miasma_kill_task > 10 then
				if inst.um_shadowcloaked_light ~= nil then
					inst.um_shadowcloaked_light:Remove()
					inst.um_shadowcloaked_light = nil
				end
...
return Prefab("um_shadow_miasma_cloud", fn, nil, prefabs),
    Prefab("um_shadow_miasma_cloud_fx", fn_fx, assets_fx)
