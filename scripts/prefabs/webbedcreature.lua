...
local function AddDurabilityLoot(inst, prefab, chance, amount)
	...
	for i = 1, (amount or 1) do
		if chance >= 1.0 or math.random() < chance then
			...
			local itemcomponent = item.components.finiteuses or item.components.fueled or item.components.perishable or item.components.armor or print(prefab .. "has no durability in webbedcreature.lua")
...
local function AddLootOnDropFn(inst, prefab, onDropFn)
    -- When this loot prefab is dropped, onDropFn is called! --Thanks Summerrr :)
    local lootDropper = inst.components.lootdropper
    if lootDropper.um_lootOnDropFns == nil then lootDropper.um_lootOnDropFns = {} end
...
local function SurvivorCocoon(inst)
	...
	elseif random_char_cocoon == "wieneke" then
        ...
        AddLootOnDropFn(inst, "koalefant_carcass", function(loot, cocoon)
            if loot.SetMeatPct == nil then return end
            loot:SetMeatPct(0.25)     -- Not sure if 25% is the right amount to have the second-to-last decay stage, might need to fiddle to get it right!
        end)
	...
	elseif random_char_cocoon == "flaire" then
        ...
        AddLootOnDropFn(inst, "spellprint", function(loot, cocoon)
            if loot.TryRevealSpell == nil then return end
	...
	elseif random_char_cocoon == "wirlywings" then
		...
		AddDurabilityLoot(inst, "cherryhat", 0.75)
		AddDurabilityLoot(inst, "cherry_cake", 0.5, 2)
		AddDurabilityLoot(inst, "cherry", nil, 2)
		AddDurabilityLoot(inst, "cherry", 0.75, 3)
		AddDurabilityLoot(inst, "cherry_double", 0.3, 3)
		AddChanceLoot(inst, "wirlycandy_regen", 0.75, 2)
		AddChanceLoot(inst, "wirlycandy_oblivious", 0.5, 2)
		AddChanceLoot(inst, "wirlycandy_blackhole", 0.3)
		AddChanceLoot(inst, "wirlycandy_goop", 0.3, 3)
