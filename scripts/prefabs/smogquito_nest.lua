...
local function SpawnQueen(inst)
	...
	if land and #TheSim:FindEntities(x1, y, z1, 5, nil, nil, { "smogquito_nest" }) < 1 then
		...
		local queen = SpawnPrefab("smogquito_nest")
    ...
    if not inst.components.health:IsDead() then
        ...
        if inst.components.childspawner ~= nil then
            ...
            for k = 1, num_to_release do
                inst.components.childspawner.childname = k <= num_warriors and "smogquito" or "smogquito"
                local smogquito = inst.components.childspawner:SpawnChild()
                if smogquito ~= nil and attacker ~= nil and smogquito.components.combat ~= nil then
                    smogquito.components.combat:SetTarget(attacker)
                    smogquito.components.combat:BlankOutAttacks(1.5 + math.random() * 2)
                end
            end
            inst.components.childspawner.childname = "smogquito"

            local emergencysmogquito = inst.components.childspawner:TrySpawnEmergencyChild()
            if emergencysmogquito ~= nil then
                emergencysmogquito.components.combat:SetTarget(attacker)
                emergencysmogquito.components.combat:BlankOutAttacks(1.5 + math.random() * 2)
            end
        end
    end
end
...
local function SpawnInvestigators(inst, data)
    if not inst.components.health:IsDead() and not (inst.components.freezable ~= nil and inst.components.freezable:IsFrozen()) then
        ...
        if inst.components.childspawner ~= nil then
            ...
            for k = 1, num_to_release do
                local smogquito = inst.components.childspawner:SpawnChild()
                if smogquito ~= nil and targetpos ~= nil then
                    smogquito.components.knownlocations:RememberLocation("investigate", targetpos)
                end
            end
        end
    end
...
local function SpreadSmog(inst)
    ...
    inst.components.timer:StartTimer("smogup", 30 * GetRandomWithVariance(0.75, 0.25))
end

local function TimerDone(inst, data)
    if data.name == "smogup" then
...
local function MakeSmogquitoNestFn(den_level)
    return function()
        ...
        inst.AnimState:SetBuild("smogquito_nest_build")
		...
		inst:DoTaskInTime(0, function(inst)
			if not inst.components.timer:TimerExists("smogup") then
				inst.components.timer:StartTimer("smogup", 30 * GetRandomWithVariance(0.75, 0.25))
			end
		end)
...
return Prefab("smogquito_nest", MakeSmogquitoNestFn(1), assets, prefabs),
    Prefab("smogquito_nest_2", MakeSmogquitoNestFn(2), assets, prefabs),
    Prefab("smogquito_nest_3", MakeSmogquitoNestFn(3), assets, prefabs)
