...
local function OnHit(inst, attacker, target)
    ...
    if inst:IsOnOcean() then
        ...
        if inst:HasTag("cannonball_incendiary") then --BOIL SOME WATER
...
--used for firing the mortarball projectile.
...
local function cannonball_sludge_fn()
    ...
    inst.persists = false
    inst.components.combat:SetAreaDamage(TUNING.CANNONBALL_SPLASH_RADIUS*2, TUNING.CANNONBALL_SPLASH_DAMAGE_PERCENT)
    ...
    inst.components.combat:SetDefaultDamage(TUNING.CANNONBALL_DAMAGE*0.66)

    return inst
end

local function cannonball_incendiary_fn()
    local inst = common_fn("cannonball_sludge", "cannonball_sludge", "spin_loop", "NOCLICK")

    inst:AddTag("cannonball_incendiary")
...
local function cannonball_sludge_item_fn()
    local inst = CreateEntity()
...
local function cannonball_incendiary_item_fn()
    local inst = CreateEntity()
    ...
    inst.entity:AddTag("boatcannon_ammo")
    inst:AddTag("cannonball_incendiary")
    inst.projectileprefab = "cannonball_incendiary"
...
return Prefab("mortarball_monkey_item", mortarball_item_fn), --only exists to fire the mortarball.
    Prefab("cannonball_sludge", cannonball_sludge_fn),
    Prefab("cannonball_incendiary", cannonball_incendiary_fn),
    Prefab("cannonball_sludge_item", cannonball_sludge_item_fn),
    Prefab("cannonball_incendiary_item", cannonball_incendiary_item_fn)
