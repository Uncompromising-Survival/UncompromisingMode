...
local function OnActivate_Clock(inst, doer)
    if inst.hourdangle and inst.seconddangle and inst.minutedangle then
        ...
        elseif inst.hourdangle.count == 1 and inst.minutedangle.count == 5 and inst.seconddangle.count == 8 then
            ...
            local launch_target = doer or inst
...
local function OnSave(inst, data)
    if inst.canbeused then
        data.canbeused = inst.canbeused
    end
end

local function OnLoad(inst, data)
    if data and data.canbeused then
...
local function OnActivate(inst)
    inst.count = inst.count >= 12 and 1 or inst.count + 1
...
local function getdesc(inst, viewer)
    local addon = viewer:HasTag("troublemaker") and "Oh, it's one of my moms carvings!\n" or ""
    return addon.."\"VII\" is engraved on the back."
end
...
return Prefab("wixie_clock", fn, assets),
    Prefab("wixie_clockhand_minute", minfn, assets),
    Prefab("wixie_clockhand_second", secfn, assets),
    Prefab("wixie_clockhand_hour", hourfn, assets),
    Prefab("wixie_clockdangley1", dangle1fn, assets),
    Prefab("wixie_clockdangley2", dangle2fn, assets),
    Prefab("wixie_clockdangley3", dangle3fn, assets),
    Prefab("wixie_clockfront", frontfn, assets),
    Prefab("wooden_queen_piece", queenfn, assets)