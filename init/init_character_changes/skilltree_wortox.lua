...
local TheGenericKV = require("generickv")
...
local function UpdateNabBags(inst)
    local item = inst.components.inventory and inst.components.inventory:GetEquippedItem(GLOBAL.EQUIPSLOTS.HANDS) or nil
    ...
    TryResetPanfluteTimer = function(inst)
        if inst.components.skilltreeupdater and inst.components.skilltreeupdater:IsActivated("wortox_panflute_playing") then
            if not inst.components.timer:TimerExists("wortox_panflute_playing") then
                inst.components.timer:StartTimer("wortox_panflute_playing", GLOBAL.GetRandomWithVariance(TUNING.SKILLS.WORTOX.WORTOX_PANFLUTE_INSPIRATION_WAIT, TUNING.SKILLS.WORTOX.WORTOX_PANFLUTE_INSPIRATION_WAIT_VARIANCE))
            end
...
if SkillTreeDefs.SKILLTREE_DEFS["wortox"] ~= nil then --in case another mod turns it nil beforehand (disabling skill tree)
    SkillTreeDefs.SKILLTREE_DEFS["wortox"] = {}
    SkillTreeDefs.CreateSkillTreeFor("wortox", skills)
    SkillTreeDefs.SKILLTREE_ORDERS["wortox"] = ORDERS
end