...
local WALTER_SKILL_STRINGS = GLOBAL.STRINGS.SKILLTREE.WALTER
...
local skills =
    {
        ...
        walter_camp_firstaid = {
            ...
            onactivate = function(inst)
				...
				inst:AddTag("um_fieldmedic") --New check for the HOT.
            ...
            ondeactivate = function(inst)
				...
				inst:RemoveTag("um_fieldmedic")
				
                if inst.components.efficientuser ~= nil then
                    inst:RemoveComponent("efficientuser") --Am I allowed to do this when vanilla didn't do it?
                end
            end,
        },
...
if SkillTreeDefs.SKILLTREE_DEFS["walter"] ~= nil then --in case another mod turns it nil beforehand (disabling skill tree)
    SkillTreeDefs.SKILLTREE_DEFS["walter"] = {}
    SkillTreeDefs.CreateSkillTreeFor("walter", skills)
    SkillTreeDefs.SKILLTREE_ORDERS["walter"] = ORDERS
end