           ...
           not CanLeaveHimBe(target, LEAVE_HIM_BE) then
            ...
            if WHAT_MY_TARGET_IS_TARGETING ~= nil and
               ...
               WHAT_MY_TARGET_IS_TARGETING.components.follower ~= nil then
                ...
                if THE_MAN_OR_THE_WOMAN ~= nil and
                   (THE_MAN_OR_THE_WOMAN:HasTag("player") or THE_MAN_OR_THE_WOMAN:HasTag("bell")) then
                    TARGET_TARGETING_PLAYER_FOLLOWERS = true
                end
            end

            if not LOOK_AT_HIM_NOT_ME and
               not TARGET_TARGETING_PLAYER_FOLLOWERS and
               not TARGET_IS_A_FOLLOWER then

                if target.prefab ~= nil and not SPECIAL_FELLOWS[target.prefab] then
           ...
           not CanLeaveHimBe(target, LEAVE_HIM_BE) then
            ...
            if not LOOK_AT_HIM_NOT_ME and
               ...
               not TARGET_IS_A_FOLLOWER then
                ...
                if target.prefab ~= nil and SPECIAL_FELLOWS[target.prefab] then
                    target.components.combat:SetTarget(inst)
                end
            end
        end
